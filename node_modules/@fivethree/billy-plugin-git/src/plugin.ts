...
export class GitPlugin {
    ...
    @Action('git commit')
    async git_commit(message: string, gitRepoPath?: string, files?: string | string[], options?: simplegit.Options) {
        const git = simplegit(gitRepoPath)
        return git.commit(message, files, options)
    }

    @Action('git push')
    async git_push(gitRepoPath?: string, remote?: string, branch?: string, options?: simplegit.Options) {
        const git = simplegit(gitRepoPath)
        return git.push(remote, branch, options)
    }

    @Action('git pull')
    async git_pull(gitRepoPath?: string, remote?: string, branch?: string, options?: simplegit.Options) {
        const git = simplegit(gitRepoPath)
