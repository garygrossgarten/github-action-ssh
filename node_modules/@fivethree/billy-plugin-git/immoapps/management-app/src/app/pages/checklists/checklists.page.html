...
<ion-content padding>
            ...
            <ion-col size="12">
                ...
                <ion-card>
                    <fiv-toolbar-search (close)="onSearchbarClose()" (fivInputChange)="applySearchFilter($event)"
                        placeholder="Name eingeben" title="Meine Vorlagen">
                    </fiv-toolbar-search>
                    <ion-card-header>
                    ...
                    <ion-card-content>
                        <table mat-table [dataSource]="dataSource" class="mat-elevation-z0" matSort>
                            ...
                            <ng-container matColumnDef="levels">
                                ...
                                <td mat-cell *matCellDef="let element">{{element.levels.length == 0 ? 'Noch keine
                                    Prüfbereiche angelegt' :
                                    element.levels.length == 1 ? '1 Prüfbereich' : element.levels.length + '
                                    Prüfbereiche'}}</td>
                            </ng-container>
    ...
    <ion-list lines="full" *fivIf="['mobile']">
        <ion-item (click)="navigateToChecklist(checklist.id)"
            *ngFor="let checklist of checklists | async | checklistsfilter : searchInput">
            <ion-label text-wrap>
                <ion-text>
