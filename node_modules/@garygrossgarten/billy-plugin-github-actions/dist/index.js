...
let GithubActionsPlugin = class GithubActionsPlugin {
    release(branch = "release") {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.exec(`git merge ${status.current}`, true);
            ...
            const remotes = yield promise_1.default().getRemotes(false);
            const remote = remotes.find(remote => remote.name === branch);
            console.log(remote
                ? `Pushing to remote ${origin}/${remote.name}`
                : `Creating remote ${origin}${branch}`);
            if (remote) {
                yield this.exec("git push", true);
            }
            else {
